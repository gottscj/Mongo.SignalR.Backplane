...
{
    [TestCaseSource(nameof(InvocationTestData)), NUnit.Framework.Category("unit")]
    public void SerializeMessages(string testName)
    {
        ...
        Assert.AreEqual(testData.SerializedCount, serializedHubMessage.Count);
        foreach (var message in serializedHubMessage)
        {
            allBytes.AddRange(message.Serialized.ToArray());
        }

        Assert.AreEqual(testData.Encoded, allBytes);
    }
    ...
    public void GlobalSupportedProtocolsOverriddenByHubSupportedProtocols()
    {
        ...
        Assert.AreEqual(1, serializedHubMessage.Count);

        Assert.AreEqual(new List<byte>() { 0x0D,
                  0x96,
                    0x01,
                    0x80,
                    0xC0,
